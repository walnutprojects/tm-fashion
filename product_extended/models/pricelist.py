# -*- coding: utf-8 -*-
from lxml.etree import use_global_python_log
...
class ProductPricelist(models.Model):
    ...
    def price_rule_where_str(self, products):
        ...
        for p in self.env['product.product'].browse(products):
            if p.default_vendor_id:
                vendor_ids.append(p.default_vendor_id.id)
        where_str = ''
        if vendor_ids:
            vendor_ids = list(vendor_ids)
            vendor_ids_str = ",".join([str(i) for i in vendor_ids])
            where_str += """ AND (item.vendor_id IS NULL OR item.vendor_id in (%s)) """ % vendor_ids_str
        return where_str

    def _compute_price_rule_get_items(self, products_qty_partner, date, uom_id, prod_tmpl_ids,
                                      prod_ids, categ_ids):
        self.ensure_one()
        params = self.price_rule_where_str(prod_ids)
        # Load all rules
        ...
        query_str += """ORDER BY
            ...
            """
        self.env.cr.execute(query_str,(prod_tmpl_ids, prod_ids, categ_ids, self.id, date, date))
...
class ProductPricelistItem(models.Model):
    ...
    applied_on = fields.Selection(selection_add=[('5_product_vendor', 'Product Vendor')],
                                  ondelete={'5_product_vendor': 'cascade'})
    ...
    def _get_pricelist_item_name_price(self):
        ...
        for item in self:
            if item.vendor_id and item.applied_on == '5_product_vendor':
    ...
    def _check_product_consistency(self):
        ...
        for item in self:
            if item.applied_on == "5_product_vendor" and not item.vendor_id:
                raise ValidationError(_("Please specify the Vendor for which this rule should be applied"))
